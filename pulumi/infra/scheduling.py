...
# -----------------------------------------------------------------------------
schedule_expression = config.config.get("cron_company_facts") or "cron(0 5 * * ? *)"
...
def _container_override_input() -> str:
    ...
    the SEC bucket's manifest; the SEC User-Agent is supplied via the task's
    environment (see ecs.py), not on the command line.
    """
    command = [
        "--sec-bucket",
        config.bucket_name,
...
schedule = aws.scheduler.Schedule(
    "idi-schedule-company-facts",
    name=f"{config.name_prefix}-schedule",
    description="Triggers the company facts processor ECS task",
